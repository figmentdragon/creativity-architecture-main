...
function metaboxes() {
	...
	// Add options metabox.
	add_meta_box( 'TheCreativityArchitect', __( 'Template Settings', 'TheCreativityArchitect' ), 'options_metabox_callback', $post_types, 'side', 'default' );
	// Add sidebar metabox.
	add_meta_box( 'sidebar', __( 'Sidebar', 'TheCreativityArchitect' ), 'sidebar_metabox_callback', $post_types, 'side', 'default' );
}
...
function options_metabox_callback( $post ) {
	...
	<h4><?php _e( 'Layout', 'TheCreativityArchitect' ); ?></h4>
	...
	<div>
		...
		<label for="layout-global"><?php _e( 'Inherit Global Settings', 'TheCreativityArchitect' ); ?></label>
		<?php
		if ( ! is_premium() ) {
	...
	<div>
		...
		<label for="layout-full-width"><?php _e( 'Full Width', 'TheCreativityArchitect' ); ?></label>
	</div>

	<div>
		<input id="layout-contained" type="radio" name="options[]" value="contained" class="layout-option" <?php checked( $width_type, 'contained' ); ?> />
		<label for="layout-contained"><?php _e( 'Contained', 'TheCreativityArchitect' ); ?></label>
	</div>
	...
	<div>
		...
		<label for="layout-custom-width"><?php _e( 'Custom Width', 'TheCreativityArchitect' ); ?></label>
	</div>
...
function post_list_columns( $columns ) {
	$columns['layout'] = __( 'Layout', 'TheCreativityArchitect' );
	return $columns;
}
...
function post_list_custom_column( $column_name, $post_id ) {
	...
	if ( in_array( 'full-width', $post_options, true ) ) {
		...
		$column_value = __( 'Full Width', 'TheCreativityArchitect' );
	} elseif ( in_array( 'contained', $post_options, true ) ) {
		$layout = 'contained';

		$column_value = __( 'Contained', 'TheCreativityArchitect' );
	} else {
		$layout = 'layout-global';

		$column_value = __( 'Inherit Global Settings', 'TheCreativityArchitect' );
	}
