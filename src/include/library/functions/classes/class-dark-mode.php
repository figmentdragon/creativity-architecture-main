...
class Dark_Mode {
	...
	public function customizer_controls( $wp_customize ) {
		...
		if ( is_object( $colors_section ) ) {
			$colors_section->title = __( 'Colors & Dark Mode', 'TheCreativityArchitect' );
		}
		...
		$description  = '<p>';
		$description .= sprintf(
			/* translators: %s: TheCreativityArchitect support article URL. */
			__( 'Dark Mode is a device setting. If a visitor to your site requests it, your site will be shown with a dark background and light text. <a href="%s">Learn more about Dark Mode.</a>', 'TheCreativityArchitect' ),
			esc_url( __( 'https://wordpress.org/support/article/TheCreativityArchitect/#dark-mode-support', 'TheCreativityArchitect' ) )
		);
		$description .= '</p>';
		$description .= '<p>' . __( 'Dark Mode can also be turned on and off with a button that you can find in the bottom corner of the page.', 'TheCreativityArchitect' ) . '</p>';
		...
		$wp_customize->add_control(
			...
			array(
				'type'            => 'checkbox',
				'section'         => 'colors',
				'label'           => esc_html__( 'Dark Mode support', 'TheCreativityArchitect' ),
				'priority'        => 110,
	...
	public function the_html( $attrs = array() ) {
		...
		printf(
			...
			esc_html__( 'Dark Mode: %s', 'TheCreativityArchitect' ),
		...
		<style>
			...
			#dark-mode-toggler > span::before {
				content: '<?php esc_attr_e( 'Off', 'TheCreativityArchitect' ); ?>';
			}
			#dark-mode-toggler[aria-pressed="true"] > span::before {
				content: '<?php esc_attr_e( 'On', 'TheCreativityArchitect' ); ?>';
			}
	...
	public function add_privacy_policy_content() {
		...
		$content = '<p class="privacy-policy-tutorial">' . __( 'TheCreativityArchitect uses LocalStorage when Dark Mode support is enabled.', 'TheCreativityArchitect' ) . '</p>'
				. '<strong class="privacy-policy-tutorial">' . __( 'Suggested text:', 'TheCreativityArchitect' ) . '</strong> '
				. __( 'This website uses LocalStorage to save the setting when Dark Mode support is turned on or off.<br> LocalStorage is necessary for the setting to work and is only used when a user clicks on the Dark Mode button.<br> No data is saved in the database or transferred.', 'TheCreativityArchitect' );
		wp_add_privacy_policy_content( 'TheCreativityArchitect', wp_kses_post( wpautop( $content, false ) ) );
	}
