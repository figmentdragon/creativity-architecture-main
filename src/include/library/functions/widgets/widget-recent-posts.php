...
class Recent_Post_Widget extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct(
			...
			esc_html__('THEMENAME - Recent Posts', 'TheCreativityArchitect'), // Name
			array( 'description' => esc_html__( 'The most recent posts on your blog.', 'TheCreativityArchitect' ), ) // Args
		);
	}
...
?>
	...
	<p>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'ignore_sticky' )); ?>">
			<?php esc_html_e( 'Ignore sticky posts', 'TheCreativityArchitect' ); ?>
		</label>
	</p>

	<p>
		<label for="<?php echo esc_attr($this->get_field_id( 'limit' )); ?>"><?php esc_html_e('Show up to', 'TheCreativityArchitect'); ?>:</label><br />
		<input type="number" step="1" min="-1" id="<?php echo esc_attr($this->get_field_id( 'limit' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'limit' )); ?>" value="<?php echo $limit; ?>" class="widefat" style="width:50px!important" /> <?php esc_html_e('posts', 'TheCreativityArchitect'); ?>
	</p>

	<p>
		<label for="<?php echo esc_attr($this->get_field_id( 'offset' )); ?>">
			<?php esc_html_e( 'Offset', 'TheCreativityArchitect' ); ?>
		</label>
		...
		<small><?php esc_html_e( 'The number of posts to skip', 'TheCreativityArchitect' ); ?></small>
	</p>

	<p>
		<label for="<?php echo esc_attr($this->get_field_id( 'post_status' )); ?>">
			<?php esc_html_e( 'Post Status', 'TheCreativityArchitect' ); ?>
		</label>
	...
	<p>
		<label for="<?php echo esc_attr($this->get_field_id( 'order' )); ?>">
			<?php esc_html_e( 'Order', 'TheCreativityArchitect' ); ?>
		</label>
		<select class="widefat" id="<?php echo esc_attr($this->get_field_id( 'order' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'order' )); ?>" style="width:100%;">
			<option value="<?php echo esc_attr('DESC')?>" <?php if(isset($instance['order'])): selected( $instance['order'], 'DESC' ); endif; ?>><?php esc_html_e( 'Descending', 'TheCreativityArchitect' ) ?></option>
			<option value="<?php echo esc_attr('ASC')?>" <?php if(isset($instance['order'])): selected( $instance['order'], 'ASC' ); endif; ?>><?php esc_html_e( 'Ascending', 'TheCreativityArchitect' ) ?></option>
		</select>
	</p>

	<p>
		<label for="<?php echo esc_attr($this->get_field_id( 'orderby')); ?>">
			<?php esc_html_e( 'Orderby', 'TheCreativityArchitect' ); ?>
		</label>
		<select class="widefat" id="<?php echo esc_attr($this->get_field_id( 'orderby')); ?>" name="<?php echo esc_attr($this->get_field_name( 'orderby' )); ?>" style="width:100%;">
			<option value="<?php echo esc_attr('ID')?>" <?php if(isset($instance['orderby'])): selected( $instance['orderby'], 'ID' ); endif; ?>><?php esc_html_e( 'ID', 'TheCreativityArchitect' ) ?></option>
			<option value="<?php echo esc_attr('author')?>" <?php if(isset($instance['orderby'])): selected( $instance['orderby'], 'author' );  endif; ?>><?php esc_html_e( 'Author', 'TheCreativityArchitect' ) ?></option>
			<option value="<?php echo esc_attr('title')?>" <?php if(isset($instance['orderby'])): selected( $instance['orderby'], 'title' );  endif; ?>><?php esc_html_e( 'Title', 'TheCreativityArchitect' ) ?></option>
			<option value="<?php echo esc_attr('date')?>" <?php if(isset($instance['orderby'])): selected( $instance['orderby'], 'date' ); endif; ?>><?php esc_html_e( 'Date', 'TheCreativityArchitect' ) ?></option>
			<option value="<?php echo esc_attr('modified')?>" <?php if(isset($instance['orderby'])): selected( $instance['orderby'], 'modified' ); endif; ?>><?php esc_html_e( 'Modified', 'TheCreativityArchitect' ) ?></option>
			<option value="<?php echo esc_attr('rand')?>" <?php if(isset($instance['orderby'])): selected( $instance['orderby'], 'rand' ); endif; ?>><?php esc_html_e( 'Random', 'TheCreativityArchitect' ) ?></option>
			<option value="<?php echo esc_attr('comment_count')?>" <?php if(isset($instance['orderby'])): selected( $instance['orderby'], 'comment_count' ); endif; ?>><?php esc_html_e( 'Comment Count', 'TheCreativityArchitect' ) ?></option>
			<option value="<?php echo esc_attr('menu_order')?>" <?php if(isset($instance['orderby'])): selected( $instance['orderby'], 'menu_order' ); endif; ?>><?php esc_html_e( 'Menu Order', 'TheCreativityArchitect' ) ?></option>
		</select>
	</p>
	...
	<p>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'show_cat')); ?>">
			<?php esc_html_e( 'Display post category(ies)', 'TheCreativityArchitect' ); ?>
		</label>
	</p>
	...
	<p>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'show_author' )); ?>">
			<?php esc_html_e( 'Display post author(s)', 'TheCreativityArchitect' ); ?>
		</label>
	</p>
	...
	<p>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'show_date' )); ?>">
			<?php esc_html_e( 'Display Date', 'TheCreativityArchitect' ); ?>
		</label>
	</p>
...
<?php
	...
	function widget($args, $instance) {
		...
		if ( $posts->have_posts() ) :
			...
			echo '<div class="entry-list '.$class_thumb.'">';
				...
				while ( $posts->have_posts() ) : $posts->the_post();
					...
					echo '<article class="entry">';
						...
						echo '<h4 class="entry-title"><a href="' . esc_url( get_permalink() ) . '" title="' . sprintf( esc_attr__( 'Permalink to %s', 'TheCreativityArchitect' ), the_title_attribute( 'echo=0' )) . '" rel="bookmark">' .  get_the_title() . '</a></h4>';
						...
						echo '<div class="entry-meta">';
							if(isset($instance['show_author']) && $instance['show_author'] == 1):
								...
								echo '<span class="entry-author">'.__('By','TheCreativityArchitect').' <a itemprop="name" href="'. get_author_posts_url(get_the_author_meta('ID')).'">'.get_the_author_meta('display_name').'</a> </span>';
							endif;
							if(isset($instance['show_date']) && $instance['show_date'] == 1):

								echo '<span class="entry-date">'.__('on','TheCreativityArchitect').' <a href="'.get_month_link(get_the_time('Y'), get_the_time('m')).'">'.get_the_time($date_format).'</a> </span>';
			...
			echo '<p>'.esc_html__('No post found.', 'TheCreativityArchitect').'</p>'."\n";
		endif;
