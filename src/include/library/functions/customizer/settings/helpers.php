...
function sort_sections_list( $wp_customize ) {
	foreach ( $wp_customize->sections() as $section_key => $section_object ) {
		if ( false !== strpos( $section_key, 'TheCreativityArchitect' ) && 'important_links' !== $section_key ) {
			$options[] = $section_key;
		}
	}
...
function section_visibility_options() {
	$options = array(
		...
		'disabled'    => esc_html__( 'Disabled', 'TheCreativityArchitect' ),
	);
...
function sections_layout_options() {
	$options = array(
		'layout-one'   => esc_html__( '1 column', 'TheCreativityArchitect' ),
		'layout-two'   => esc_html__( '2 columns', 'TheCreativityArchitect' ),
		'layout-three' => esc_html__( '3 columns', 'TheCreativityArchitect' ),
		'layout-four'  => esc_html__( '4 columns', 'TheCreativityArchitect' ),
	);
...
function section_type_options() {
	$options = array(
		'post'     => esc_html__( 'Post', 'TheCreativityArchitect' ),
		'page'     => esc_html__( 'Page', 'TheCreativityArchitect' ),
		'category' => esc_html__( 'Category', 'TheCreativityArchitect' ),
		'custom'   => esc_html__( 'Custom', 'TheCreativityArchitect' ),
	);
...
function get_pagination_types() {
	$pagination_types = array(
		'default' => esc_html__( 'Default(Older Posts/Newer Posts)', 'TheCreativityArchitect' ),
		'numeric' => esc_html__( 'Numeric', 'TheCreativityArchitect' ),
	);
...
function generate_post_array( $post_type = 'post' ) {
	...
	$output['0']= esc_html__( '-- Select --', 'TheCreativityArchitect' );

	foreach ( $posts as $post ) {
		$output[ $post->ID ] = ! empty( $post->post_title ) ? $post->post_title : sprintf( __( '#%d (no title)', 'TheCreativityArchitect' ), $post->ID );
	}
...
function content_show() {
	$options = array(
		'excerpt'      => esc_html__( 'Show Excerpt', 'TheCreativityArchitect' ),
		'full-content' => esc_html__( 'Full Content', 'TheCreativityArchitect' ),
		'hide-content' => esc_html__( 'Hide Content', 'TheCreativityArchitect' ),
	);
...
function meta_show() {
	$options = array(
		'show-meta' => esc_html__( 'Show Meta', 'TheCreativityArchitect' ),
		'hide-meta' => esc_html__( 'Hide Meta', 'TheCreativityArchitect' ),
	);
