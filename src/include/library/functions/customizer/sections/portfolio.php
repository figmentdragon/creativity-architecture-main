...
function portfolio_options( $wp_customize ) {
	...
	register_option( $wp_customize, array(
			...
			'label'             => sprintf( esc_html__( 'For Portfolio Options for THEMENAME Theme, go %1$shere%2$s', 'TheCreativityArchitect' ),
	...
	$wp_customize->add_section( 'portfolio', array(
			'panel'    => 'theme_options',
			'title'    => esc_html__( 'Portfolio', 'TheCreativityArchitect' ),
		)
	);
    ...
    register_option( $wp_customize, array(
            ...
            'label'             => sprintf( esc_html__( 'For Portfolio, install %1$sEssential Content Types%2$s Plugin with Portfolio Type Enabled', 'TheCreativityArchitect' ),
	...
	register_option( $wp_customize, array(
			...
			'active_callback'   => 'is_portfolio_active',
			'label'             => esc_html__( 'Number of items to show', 'TheCreativityArchitect' ),
			'section'           => 'portfolio',
			'type'              => 'number',
			'input_attrs'       => array(
				'style'             => 'width: 100px;',
	...
	for ( $i = 1; $i <= $number ; $i++ ) {
		...
		register_option( $wp_customize, array(
				...
				'active_callback'   => 'is_portfolio_active',
				'label'             => esc_html__( 'Portfolio', 'TheCreativityArchitect' ) . ' ' . $i ,
				'section'           => 'portfolio',
				'type'              => 'select',
