...
function slider_options( $wp_customize ) {
	$wp_customize->add_section( 'featured_slider', array(
			'panel' => 'theme_options',
			'title' => esc_html__( 'Featured Slider', 'TheCreativityArchitect' ),
		)
	);
	...
	register_option( $wp_customize, array(
			...
			'description'       => esc_html__( 'Save and refresh the page if No. of Slides is changed (Max no of slides is 20)', 'TheCreativityArchitect' ),
			'input_attrs'       => array(
				'style' => 'width: 100px;',
				'min'   => 0,
				'max'   => 20,
			...
			'label'             => esc_html__( 'No of Slides', 'TheCreativityArchitect' ),
			'section'           => 'featured_slider',
			'type'              => 'number',
		)
	);
	...
	for ( $i = 1; $i <= $slider_number ; $i++ ) {
		...
		register_option( $wp_customize, array(
				...
				'active_callback'   => 'is_slider_active',
				'label'             => esc_html__( 'Logo Image #', 'TheCreativityArchitect' ) . $i,
				'section'           => 'featured_slider',
			)
		);
		...
		register_option( $wp_customize, array(
				...
				'active_callback'   => 'is_slider_active',
				'label'             => esc_html__( 'Page', 'TheCreativityArchitect' ) . ' # ' . $i,
				'section'           => 'featured_slider',
				'type'              => 'dropdown-pages',
			)
		);
