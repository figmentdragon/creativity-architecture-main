...
function service_options( $wp_customize ) {
    ...
    register_option( $wp_customize, array(
            ...
            'label'             => sprintf( esc_html__( 'For all Services Options, go %1$shere%2$s', 'TheCreativityArchitect' ),
    ...
    $wp_customize->add_section( 'service', array(
			'title' => esc_html__( 'Services', 'TheCreativityArchitect' ),
			'panel' => 'theme_options',
		)
	);
    ...
    register_option( $wp_customize, array(
            ...
            'label'             => sprintf( esc_html__( 'For Services, install %1$sEssential Content Types%2$s Plugin with Service Type Enabled', 'TheCreativityArchitect' ),
	...
	register_option( $wp_customize, array(
			...
			'active_callback'   => 'is_services_active',
			'description'       => esc_html__( 'Save and refresh the page if No. of Services is changed (Max no of Services is 20)', 'TheCreativityArchitect' ),
			'input_attrs'       => array(
				'style' => 'width: 100px;',
				'min'   => 0,
			),
			'label'             => esc_html__( 'No of items', 'TheCreativityArchitect' ),
			'section'           => 'service',
			'type'              => 'number',
	...
	for ( $i = 1; $i <= $number ; $i++ ) {
		register_option( $wp_customize, array(
				...
				'active_callback'   => 'is_services_active',
				'label'             => esc_html__( 'Services', 'TheCreativityArchitect' ) . ' ' . $i ,
				'section'           => 'service',
				'type'              => 'select',
